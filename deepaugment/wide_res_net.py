...
def WideResidualNetwork(
    ...
    if weights == "cifar10" and include_top and classes != 10:
        raise ValueError(
            "If using `weights` as CIFAR 10 with `include_top`"
            " as true, `classes` should be 10"
        )

    if (depth - 4) % 6 != 0:
        raise ValueError(
            "Depth of the network must be such that (depth - 4)"
    ...
    x = __create_wide_residual_network(
        classes, img_input, include_top, depth, width, dropout_rate, activation
    )
            ...
            if K.image_dim_ordering() == "th":
                if include_top:
                    weights_path = get_file(
                        "wide_resnet_28_8_th_dim_ordering_th_kernels.h5",
                        TH_WEIGHTS_PATH,
                        cache_subdir="models",
                    )
                else:
                    weights_path = get_file(
                        "wide_resnet_28_8_th_dim_ordering_th_kernels_no_top.h5",
                ...
                if include_top:
                    weights_path = get_file(
                        "wide_resnet_28_8_tf_dim_ordering_tf_kernels.h5",
                        TF_WEIGHTS_PATH,
                        cache_subdir="models",
                    )
                else:
                    weights_path = get_file(
                        "wide_resnet_28_8_tf_dim_ordering_tf_kernels_no_top.h5",
...
def __create_wide_residual_network(
    nb_classes,
    img_input,
    include_top,
    depth=28,
